...
| Board deck, slide S5 filled in | Done | `wiki/project/Board-Presentation-Sweetgreen.pptx` — the annotated map and one-line consequence dropped into the official `IE-GY_9113B_Board_Presentation_Template.pptx`'s S5 (Backbone diagnosis) slide; every other slide stays a placeholder for later weeks |
| Standalone Lab 1 presentation (map, curveball, data, consequence) | Done | `wiki/project/Lab1-Presentation-Sweetgreen.pptx` — a self-contained 5-slide deck for the in-class 6-minute share-out, separate from the semester-long board deck above |
