...
- `raw/data/scratch-notes/`: unpolished raw working notes that aren't a transcript of anyone else's words — `Sweetgreen-Breakout-Scratch-Notes.md` (raw in-class breakout work). Project-specific; moved out of `transcripts/` since it doesn't belong there (it's the team's own scratch work, not a record of external communication).
...
- `wiki/project/`: the team's Sweetgreen capstone work. Backbone-context note, context-and-diagnosis brief, Lab 1 pre-work brief, the Lab 1 process map, the RPA-candidate scan, a research log under `wiki/project/research/`, standalone diagram source files under `wiki/project/diagrams/` (embedded into write-up notes with `![[...]]` rather than pasted inline, so a diagram can be edited in one place), and the living presentation decks (`.pptx` + `.pdf` export, filled in slide by slide as each lab/week produces its evidence).

## Capstone context

- **Company:** [[Sweetgreen]], a fast-casual restaurant chain. Confirmed in Week 2. An earlier Week 1 practice exercise used AWS as a throwaway drill, not the real company. See the note at the top of `raw/data/scratch-notes/Sweetgreen-Breakout-Scratch-Notes.md`.
