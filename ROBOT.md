...
- **Course study notes and lecture slides still get converted to Markdown** (`markitdown`, original deleted) so they're searchable and git-diffable like everything else in `w0`–`w3`. PDFs are otherwise fine in this vault: presentation decks live as both `.pptx` and a `.pdf` export (e.g. `Lab1-Presentation-Sweetgreen.pptx` / `.pdf`), and other reference PDFs can just sit in `project/` or `Final Project Info/` as-is.
